...
- Tip: **Performance:** Be careful with `ToList()` too early; it pulls everything into memory.
- Security: **Non-mutating pipelines:** LINQ operations do not modify the source collection; they produce a new sequence definition or result.
...
**Gotchas:**
- Caution: **Multiple Enumeration:** Iterating a LINQ query twice (e.g., `Count()` then `foreach`) re-executes the query logic. Use `ToList()` or `ToArray()` to materialize if needed multiple times.
- Caution: **Capturing Variables:** Be aware of closure semantics when lambdas capture changing variables from surrounding code.
- Caution: **Client-side Evaluation:** In EF Core, some LINQ methods cannot be translated to SQL and will throw or pull data locally (implicit client eval is disabled in modern EF Core).
- Caution: **Hot paths:** LINQ readability is often worth it, but tight loops can pay measurable delegate, iterator, and allocation costs.
...
Method and query syntax are alternative LINQ forms; choose the clearer expression for the operation and provider.
...
1.  **Materialization:** Materialize deliberately when a stable snapshot or repeated enumeration is needed.
...
LINQ-to-Objects iterators often defer work, but terminal operators execute when called and some operators buffer. IQueryable providers translate supported expression trees and can issue SQL; test translation and inspect SQL for EF Core. IAsyncEnumerable streams asynchronously only when its provider and operators allow it. Method and query syntax are alternative styles, not a universal ranking.

## Related Concepts
- [[Generic Constraints]] - LINQ relies heavily on generics.
- [[Delegates, Events, and Actions]] - `Func<T, bool>` predicates are the backbone of LINQ.
- [[Expression-Bodied Members]] - Often used with LINQ selectors.
- [[Collection Types]] - Source collection choice affects evaluation, materialization, and performance.
