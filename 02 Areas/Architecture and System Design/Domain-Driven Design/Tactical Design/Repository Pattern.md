...
- To expose a domain-focused persistence boundary; test database query behavior with relational integration tests.
...
- **Testability:** A fake can isolate domain logic, but it does not validate provider queries or transactions.
- **Consistency:** Prevents scattered LINQ queries throughout the codebase.
- Tip: **Performance:** Be careful not to hide performance pitfalls (like N+1 problems) behind the abstraction.
...
**Gotchas:**
- Caution: **Generic Repository Trap:** Avoid `IRepository<T>` for everything. It often leads to leaky abstractions (`IQueryable` leaking out). Prefer specific repositories (`IUserRepository`) or a hybrid approach.
- Caution: **IQueryable Leaks:** Returning `IQueryable<T>` from a repository defeats the purpose of the pattern, as the controller or service can modify the query (coupling them to the DB schema).
- Caution: **Over-abstraction:** With EF Core, the `DbSet` is already a repository and `DbContext` is a Unit of Work. For simple apps, a repository layer might be overkill.
...
| :--- | :--- | :--- | :--- |
| **Complexity** | Often fewer abstractions | Adds a boundary and implementation | Adds separate command/query paths |
| **Testing** | Integration tests can exercise real queries | Unit tests can fake the boundary; integration tests still needed | Handler tests plus provider integration tests |
| **Query design** | Direct provider queries | Repository API or specification | Dedicated query handlers/models |
| **Fit** | Simple application data access or query-heavy code | Domain-focused persistence boundary | Distinct command/query models when requirements justify them |
...
### Transaction and test boundary

`SaveChangesAsync` commits together only when both repositories use the same `DbContext`/transaction. Calls to a remote inventory service require separate coordination and cannot be made atomic by a local unit of work. Test a repository's LINQ translation, includes, and transaction behavior against the chosen relational provider; mocked interfaces only test the calling service. Direct `DbContext` use can be appropriate in a small application. [Microsoft EF Core persistence guidance](https://learn.microsoft.com/en-us/dotnet/architecture/microservices/microservice-ddd-cqrs-patterns/infrastructure-persistence-layer-implementation-entity-framework-core) (checked 2026-09-24).

## Related Concepts
- [[Unit of Work]] - The combination of these two patterns.
...
- [Microsoft, EF Core persistence layer and repositories](https://learn.microsoft.com/en-us/dotnet/architecture/microservices/microservice-ddd-cqrs-patterns/infrastructure-persistence-layer-implementation-entity-framework-core) — online page, date not shown; accessed 2026-09-24.
