...
**When to use:**
- Systems where read and write needs diverge significantly.
...
- Caution: **Added complexity**: consistency, operations, and debugging get harder.
- Caution: **Not mandatory for every app**: simple CRUD systems usually do better with a simpler architecture.
...
A derived read model can be rebuilt only if the authoritative state or event history is complete and a rebuild process exists.

### Consistency Model
CQRS alone does not require eventual consistency. Asynchronously updated read projections can lag behind committed writes.
...
A monolith can use separate `ApproveOrder` command and `OrderSummary` query handlers against one transactional database. Both can see committed writes without an asynchronous projection. Add a separate read store only when measured query or scaling needs justify synchronization, monitoring, replay, and stale-read policy. CQRS does not require Event Sourcing. [Microsoft CQRS pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/cqrs) (updated 2025-02-21; checked 2026-09-24).
...
- [Microsoft, CQRS pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/cqrs) — last updated 2025-02-21; accessed 2026-09-24.
