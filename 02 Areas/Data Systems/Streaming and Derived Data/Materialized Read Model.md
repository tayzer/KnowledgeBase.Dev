...
- **Design for rebuild where possible:** replay and correction require retained, accessible authoritative records/events.
- Caution: **Freshness is part of the contract:** define acceptable lag, expose timestamps when useful, and monitor projection delay.
- Caution: **Not a business authority:** do not let the read model become the place where command-side rules secretly live.

**Limit:** Rebuild requires retained authoritative inputs; stale views need an explicit freshness contract.
...
## Rebuild and freshness precondition

A read model can be rebuilt only if authoritative records or events are retained and replayable for the required span. For an order-summary projection, consume order changes, upsert by event ID/version, expose last-updated time, and reconcile missed messages from the source. An outbox can coordinate record changes and message publication, but delivery still requires idempotent consumers and a lag policy.
...
- [Fowler CQRS](https://martinfowler.com/bliki/CQRS.html) (accessed 2026-09-24).

- [Primary documentation](https://learn.microsoft.com/en-us/azure/architecture/patterns/materialized-view) (accessed 2026-09-24).
- [Microsoft Docs: CQRS pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/cqrs) (accessed 2026-09-24).
- [Martin Fowler: CQRS](https://martinfowler.com/bliki/CQRS.html) (accessed 2026-09-24).
- [Martin Fowler: Reporting database](https://martinfowler.com/bliki/ReportingDatabase.html) (accessed 2026-09-24).
