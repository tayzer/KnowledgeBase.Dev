---
date: 2026-04-30
status: Current
tags:
  - aws
  - s3
  - object-storage
  - cloud

---

# Amazon S3

## Quick Reference

**Definition:** AWS object storage service for durable, scalable storage of files, blobs, backups, archives, and data-lake assets.

**When to use:**
- Storing unstructured files such as media, exports, backups, logs, and static assets.
- Building durable object-storage layers for analytics, archival, or event-driven workflows inside AWS.

**Key Takeaways:**
- **Documented durability and scale** make S3 a candidate for file-based workloads; compare access, retrieval, and cost needs.
- **Lifecycle, versioning, and IAM integration** support long-term operational control.
- Caution: **It is object storage**: design around objects, prefixes, and policies rather than filesystem or database assumptions.

**Limit:** S3 object semantics, lifecycle retrieval, and charges differ from filesystem or database semantics.

---

## Deep Dive

### Good Fit
S3 works well for application uploads, backup storage, static website assets, analytics staging, and long-term archive strategies.

### Design Considerations
- Model access around object keys and bucket policies rather than directory semantics.
- Pay attention to request patterns, lifecycle rules, retention, and retrieval or egress costs.
- Keep security boundaries explicit with IAM, bucket policies, and encryption settings.

## Consistency, access, and cost

Amazon S3 provides strong read-after-write consistency for object PUT and DELETE operations, with documented caveats for concurrent writes. Versioning and lifecycle rules address different goals; lifecycle transitions and retrieval classes affect access time and charges. Scope bucket policy/IAM, encryption, retry, overwrite, and egress decisions to the workload. S3 is object storage, not a general filesystem or database transaction layer.

## Sources

- [Primary documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Welcome.html) (accessed 2026-09-24).

## Related Concepts
- [[Cloud Storage Services]]
- [[Data Warehouses]]

## Review Schedule
- [ ] Review in 3 months
- Draft fact-check: 2026-09-24. Set the next dated review when promoted.
