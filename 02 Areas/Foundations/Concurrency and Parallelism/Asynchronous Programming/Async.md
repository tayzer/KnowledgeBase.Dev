...
**Definition:** Async/await is a structured way to model asynchronous work as awaitable operations, usually represented by tasks, futures, or promises, when the awaited operation is genuinely asynchronous; syntax alone does not eliminate blocking.
...
- Exceptions from a Task-returning async method fault the task and surface on await; pre-return validation can throw synchronously.
...
Async syntax alone does not make blocking work nonblocking. In C#, awaiting genuinely asynchronous I/O releases the current worker while waiting. Task.WhenAll waits for every supplied task; a fault takes precedence over cancellation in its aggregate result. Inspect input tasks if partial results matter, and bound fan-out by downstream capacity.
...
- [Primary documentation](https://learn.microsoft.com/en-us/dotnet/csharp/asynchronous-programming/async-scenarios) (accessed 2026-09-24).
