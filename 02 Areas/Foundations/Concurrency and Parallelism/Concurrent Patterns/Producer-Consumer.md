...
For an in-memory bounded channel, stop producers, complete its writer, and drain the reader during graceful shutdown. An accepted in-process write is not durable storage; broker acknowledgment behavior depends on the selected product and configuration.
