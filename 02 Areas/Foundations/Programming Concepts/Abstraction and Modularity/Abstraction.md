...
2. **Interfaces** – Contracts that may include default members in modern C#.

**When to use:**
- Simplifying complex systems by exposing only relevant operations.
- Defining contracts between layers (e.g., domain services and [[Data Access]]).
- Enabling [[Polymorphism]] through common interfaces.

**Key Takeaways:**
- Expose **what**, not **how**; hide implementation details.
- Use interfaces for independent contract implementers; use abstract classes for shared base behavior or state.
- Encourages [[Dependency Injection]] – depend on abstractions, not concretions.
- Caution: Over-abstraction creates unnecessary indirection; abstract only when variation or change is anticipated.

**Code Snippet:**
```csharp
// Preferred: Abstraction: Hide database complexity
...
**Gotchas:**
- Caution: **Leaky abstraction:** Exposing too many implementation details defeats the purpose (e.g., SQL exception types bubbling up from a repository).
- Caution: **Over-engineering:** A single implementation can justify a stable boundary; avoid indirection without a clear benefit.
- Caution: **Abstraction mismatch:** Interfaces that don't match actual usage patterns create awkward conversions.
...
```csharp
// Avoid: Leaky abstraction: Exposes SQL details
public interface IUserRepository
{
    SqlDataReader ExecuteQuery(string sql);
}

// Preferred: Good abstraction: Exposes domain concepts
public interface IUserRepository
{
    User GetById(int id);
...
Introduce an abstraction for a meaningful boundary or likely variation:

```csharp
// Avoid: Over-engineering: Single implementation
...
// Simpler option when a separate contract adds no value
...
A single implementation can still justify an interface when it defines a stable dependency boundary. Multiple implementations do not justify an interface if callers need incompatible contracts. Modern C# interfaces can provide default members; choose an abstract class when shared base state or implementation is needed.
...
- [[Dependency Injection]] – Leverages abstractions to inject implementations.
- [[SOLID Principles|DIP]] – Formalized rule: depend on abstractions, not concretions.

---

## Sources

- [Primary documentation](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/types/interfaces) (accessed 2026-09-24).
