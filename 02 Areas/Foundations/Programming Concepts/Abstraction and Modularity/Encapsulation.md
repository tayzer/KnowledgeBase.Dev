...
**Core Principle:** Give callers only the operations needed to keep a type's invariants valid. Immutable public values can be exposed directly when they cannot corrupt those invariants.

**When to use:**
- Use it when a type must protect invariants or hide implementation changes.
...
- Preserves a stable public contract independently of [[Polymorphism]] – internal implementation can change without affecting external interface.
- Caution: Excessive accessors or forwarding methods can obscure a simple data contract; choose visibility from the actual invariant and API needs.

**Code Snippet:**
```csharp
// Preferred: Proper encapsulation
...
// Avoid: Poor encapsulation (exposes internal state)
...
**Gotchas:**
- Caution: **Public fields:** Prefer controlled access where mutable state has invariants; simple immutable data has different tradeoffs.
- Caution: **Over-accessors:** Providing getters/setters for everything nullifies encapsulation benefits.
- Caution: **Leaky abstractions:** Returning mutable reference types can allow external modification (return copies or immutable collections).
...
| `private protected` | Same class or derived in same assembly | Useful for a deliberately narrow inheritance API.        |
|                     |                                        |                                                          |
|                     |                                        |                                                          |
...
Use encapsulation where a type must protect invariants or implementation changes. A simple immutable value may be safe to expose directly. Visibility alone does not enforce an invariant, and encapsulation is distinct from polymorphism.
...
- [[SOLID Principles|DIP]] – Encapsulation supports depending on abstractions, not implementations.

---

## Sources

- [Primary documentation](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/access-modifiers) (accessed 2026-09-24).
